...
基础检索规划不一定需要 API key。没有 key 时，仍然可以生成检索式、人工补查路线、候选文献表和审计清单。需要调用 Kimi、TinyFish、AnySearch、Grok/X 等增强检索源时，再检查对应凭证。
...
不要把真实 key 写进提示词、报告、脚本、示例、命令历史、README、`SKILL.md` 或任何公开项目文档。如果用户把 key 粘贴到对话里，只能用于存入私密凭证位置，并建议轮换。
